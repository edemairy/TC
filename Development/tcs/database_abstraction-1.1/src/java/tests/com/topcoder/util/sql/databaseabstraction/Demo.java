 ...
 * </p>
 * <p>
 * Get corresponding Objects from retrieved JDBC ResultSet, if mapper is provided, that the value will be
 * converted to the corresponding value using specified converter, when retrieving values, it will looking up
 * it in the mapped value , this is what this component behaves in version 1.0.
 * </p>
 * <p>
 * In version 1.1, the component stores the original value, so it is possible to look up desired values if it
 * is the same type with that of original value. And this component also deals with the situation that the
 * desired type is not found in original or mapped value. Here on-demand converter is configured in
 * DatabseAbstractor. once on-demand converter is provided, this component will use the on-demand converter to
 * dynamically convert the mapped value or original value to the desired type when desired value can't be
 * found in mapped value.
 * </p>
 *
 * @author justforplay
 * @version 1.1
...
public class Demo extends TestCase {
    /**
     * Demo the usage of component Database abstraction.Including creation of CustomResultSet and factory
     * class - DatabaseAbstractor, Retrieving value from original value, mapped value, Retrieving values when
     * On-Demand conversion is needed.
     *
    ...
    public void testDemo1() throws Exception {
        ...
        try {
            ...
            // Class MockConverterBoolean which converts BigDecimal value to Boolean(true)
            ...
            // create mapper with one converter which aims to convert number column to boolean.
            ...
            while (crs2.next()) {
                ...
                // Get not exist value in original value or mapped value, but was converted from BigDecimal to
                // Byte.
                crs.getByte("ID");
                // Get not exist value in original value or mapped value, but was converted from BigDecimal to
                // String.
