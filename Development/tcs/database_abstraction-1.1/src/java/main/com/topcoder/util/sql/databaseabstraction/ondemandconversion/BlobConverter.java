...
/**
 * <p>
 * The BlobConverter class handles on demand conversions from the Blob type to various other data types. It
 * implements the OnDemandConverter interface and has no state.
 * </p>
 * <p>
 * When a value is converted by BlogConverter, the condition that value can be converted is: value is a Blob
 * and is not null, and desiredType is InputStream or byte[].
 * </p>
 * <p>
...
public class BlobConverter implements OnDemandConverter {
     ...
     * value is a Blob and is not null, and desiredType is InputStream or byte[].
     * </p>
     *
    ...
    /**
     * <p>
     * Converts value into an instance of desiredType. If value is not a Blob and is not null, and desiredType
     * is InputStream or byte[], value is converted to the corresponding type and returned.
     * </p>
     *
     ...
     *             count of metadata
     * @throws IllegalMappingException If the conversion can not be made (if either of the Blob calls results
     *             in a SQL exception)
     */
    public Object convert(Object value, int column, CustomResultSetMetaData metaData, Class desiredType)
        ...
        try {
            ...
            if (desiredType.isAssignableFrom(InputStream.class)) {
                ...
                return blobValue.getBytes((long) 1, (int) blobValue.length());
            }
