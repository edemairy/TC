...
/**
 * <p>
 * The StringConverter class handles on demand conversions from the String type to various other data types.
 * It implements the OnDemandConverter interface and has no state.
 * </p>
 * <p>
 * When a value is converted by StringConverter, the condition that value can be converted is: value is a
 * String and not null, and desiredType is java.io.InputStream and characters in String are all &gt;=0 and
 * &lt;128,or desiredType is java.io.Reader, or desiredType is Byte, Short, Integer, Long, Double, Float, or
 * BigDecimal and String value can be parsed to the desired type, or desiredType is Date, Timestamp, or Time,
 * and String value can be converted to corresponding type(date format is used to parse the String is it is
 * not null).
 * </p>
 * <p>
...
public class StringConverter implements OnDemandConverter {
    ...
    /**
     * The format used when converting a String value into a Date. If it is null, the valueOf methods will be
     * used to parse dates instead. This field is set in the constructor, is immutable, and can be null.
     */
     ...
     *
     * @param dateFormat The format to use when parsing dates
    ...
    /**
     * <p>
     * Tells whether value can be converted to desiredType.The condition that value can be converted is: value
     * is a String and not null, and desiredType is java.io.InputStream and characters in String are all
     * &gt;=0 and &lt;128,or desiredType is java.io.Reader, or desiredType is Byte, Short, Integer, Long,
     * Double, Float, or BigDecimal and String value can be parsed to the desired type, or desiredType is
     * Date, Timestamp, or Time, and String value can be converted to corresponding type(date format is used
     * to parse the String is it is not null).
     * </p>
     *
    ...
    public boolean canConvert(Object value, int column, CustomResultSetMetaData metaData, Class desiredType) {
        ...
        try {
            ...
            // thrown by parse method of Byte,Short, Integer, Long, Double, Float
    ...
    /**
     * <p>
     * Converts value into an instance of desiredType. If value is a String and not null, and desiredType is
     * java.io.InputStream and characters in String are all &gt;=0 and &lt;128;or desiredType is
     * java.io.Reader; or desiredType is Byte, Short, Integer, Long, Double, Float, or BigDecimal and String
     * value can be parsed to the desired type; or desiredType is Date, Timestamp, or Time, and String value
     * can be converted to corresponding type(date format is used to parse the String is it is not null),value
     * is converted to desired type and returned.
     * </p>
     *
     ...
     * @throws IllegalArgumentException If metaData or desiredType is null or column is <= 0 or > column count
     *             of metadata
    ...
    public Object convert(Object value, int column, CustomResultSetMetaData metaData, Class desiredType)
        ...
        // canConvert is not invoked here, for invoking it will parse the value twice.
        String valueStr = (String) value;
        if (desiredType.isAssignableFrom(InputStream.class)) {
            ...
            for (int i = 0; i < valueStr.length(); i++) {
                if (valueStr.charAt(i) >= 0 && valueStr.charAt(i) <= MAX_CHAR_VALUE) {
                    ...
                    throw new IllegalMappingException("value can't be converted to the desired type -- "
                        + desiredType + " by StringConverter, character in String is <0 or >127.");
                }
            }
