...
/**
 * <p>
 * The ByteArrayConverter class handles on demand conversions from byte arrays to various other data types. It
 * implements the OnDemandConverter interface and has no state.
 * </p>
 * <p>
 * When a value is converted by ByteArrayConverter, the condition that value can be converted is: value is a
 * byte[] and is not null, and desiredType is InputStream.
 * </p>
 * <p>
...
public class ByteArrayConverter implements OnDemandConverter {
     ...
     * value is a byte[] and is not null, and desiredType is InputStream.
     * </p>
     *
    ...
    public boolean canConvert(Object value, int column, CustomResultSetMetaData metaData, Class desiredType) {
        ...
        return AbstractionHelper
            .canConvert(value, byte[].class, desiredType, new Class[] {InputStream.class});
    }

    /**
     * <p>
     * Converts value into an instance of desiredType. If value is a byte[] and is not null, and desiredType
     * is InputStream,value is converted to the corresponding type and returned.
     * </p>
     *
    ...
    public Object convert(Object value, int column, CustomResultSetMetaData metaData, Class desiredType)
        ...
        if (!canConvert(value, column, metaData, desiredType)) {
            throw new IllegalMappingException("value can't be converted to the desired type -- "
                + desiredType + " -- by ByteArrayConverter.");
        }
